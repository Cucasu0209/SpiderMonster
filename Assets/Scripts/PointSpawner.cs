...
public class PointSpawner : MonoBehaviour
{
    ...
    private void Awake()
    {
        EventDispatcher.Addlistener(ScriptName.Spanwer, Events.RespawnPoint, SpawnPointsOverScreen);
    ...
    public void SpawnPointsOverScreen()
    {
        List<Vector2> positions = GetPointPos(Camera.main.transform.position, (Camera.main.transform.position - Camera.main.ScreenToWorldPoint(Vector3.zero)) * 2.4f, 100);
    ...
    private List<Point> GetPoins() => points;

    private List<Point> GetNearPoints(Vector3 center, float distance)
    {
        return points.Where(p => Vector2.Distance(center, p.transform.position) < distance).ToList();
    }
